<template>
  <div class="container">
    ...
    <van-cell class="cell">
      ...
      <template #default>
        <div class="service-text">予約内容</div>
      </template>
    </van-cell>
    <van-cell-group class="cell">
      <van-cell>
        <template #title>予約クリニック</template>
        <template #default>
          <div @click="showHospital=true">{{form.hospital_name || "クリニック選択"}}
            <van-icon name="arrow" />
          </div>
        </template>
      </van-cell>
      <van-cell>
        <template #title>予約時間</template>
        <template #default>
          <div @click="showStartTime=true">{{ currentDate || "日付選択" }}
            <van-icon name="arrow" />
          </div>
        </template>
      </van-cell>
      <van-cell>
        <template #title>予約医師</template>
        <template #default>
          <div @click="showComponion=true">{{ form.companion_name || "医師選択" }}
            <van-icon name="arrow" />
          </div>
        </template>
      </van-cell>
    ...
    <van-cell-group title="備考欄"
                    class="cell">
      <van-field class="text"
                 style="height: 100px;"
                 v-model="form.demand"
                 placeholder="ご希望内容を入力してください" />
    </van-cell-group>
    ...
    <van-popup v-model:show="showHospital"
               position="bottom"
               :style="{ height: '30%' }">
      <van-picker :columns="showHospColumns"
    ...
    <van-popup v-model:show="showComponion"
               position="bottom"
               :style="{ height: '30%' }">
      <van-picker :columns="showComponionColumns"
